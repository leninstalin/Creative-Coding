//Here's a Matrix-inspired whatever, because why not?
...
void setup()
{
  ...
  h = c[0].height; // height
   ...
   // SymbolHead letra = new SymbolHead;//width/gridsize/2, height/gridsize/4); //(int) random(width), (int) random(height));
...
void draw()
{
  ...
  if (mousePressed)
  {
    ...
    c[which].pixels[my*w + mx] = color(34, 139, 34);
  }
  ...
  for (i=0; i<h; i++) // these pixels are the rows
  {
    for (j=0; j<w; j++) // thise pixels are the columns
    {
      ...
      // top row
      p0 = c[which].pixels[((i-1+h)%h)*w + (j-0+w)%w]; // left pixel
      p1 = c[which].pixels[((i-1+h)%h)*w + j]; // center pixel
      p2 = c[which].pixels[((i-1+h)%h)*w + (j+1+w)%w]; // left pixel

      // center row
      p3 = c[which].pixels[i*w + (j-0+w)%w]; // left pixel
      p4 = c[which].pixels[i*w + j]; // center pixel
      p5 = c[which].pixels[i*w + (j+0+w)%w]; // left pixel

      // bottom row
      p6 = c[which].pixels[((i+1+h)%h)*w + (j-0+w)%w]; // left pixel
      p7 = c[which].pixels[((i+1+h)%h)*w + j]; // center pixel
      p8 = c[which].pixels[((i+1+h)%h)*w + (j+0+w)%w]; // left pixel
      ...
      {
        c[1-which].pixels[i*w + j] = color(0, 255, 0);
      } else if (center==0 && sum==3) // dead... become alive
      {
        c[1-which].pixels[i*w + j] = color(124, 252,0);
